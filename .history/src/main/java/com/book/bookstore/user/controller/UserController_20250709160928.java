...
public class UserController {
    ...
    public ResponseEntity<?> login(@RequestBody UserLoginRequest request, HttpSession session) {
        ...
        // 비밀번호 검증(실제 서비스에서는 암호화 필요)
        // 여기서는 password 필드가 User에 없으므로, 실제 구현 시 User에 password 필드 추가 필요
    ...
    public ResponseEntity<?> resetPassword(@RequestBody PasswordResetRequest request) {
        ...
        // 실제로는 임시 비밀번호 생성 및 이메일 전송 로직 필요
        // 여기서는 예시로 임시 비밀번호를 "temp1234"로 설정
        User user = userOpt.get();
        // user.setPassword(passwordEncoder.encode("temp1234")); // 실제 구현 시 암호화 필요
        userService.updateUser(user.getId(), user); // 임시 비밀번호 저장
